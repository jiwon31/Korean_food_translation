...
lastpage = async(page) => {
    await page.goto("https://lampcook.com/food/food_dic_menu1.php");
...
getdata = async() => {
    ...
    var URL = "https://lampcook.com/food/food_dic_menu1.php?pagenum=";
    ...
    for(var i=1; i<=max; i++){
        ...
        for(var j=6; j<maxx; j++){
           ...
           // await page.click(`#div_main_content > div:nth-child(${2*j+5}) > div > ul > li:nth-child(2) > a`);
            //await page.waitForSelector("#div_main_content > div:nth-child(5) > h1");
    ...
    fs.writeFileSync('koofood.json', JSON.stringify(dataset));
    await browser.close();
}
