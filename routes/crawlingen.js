...
lastpage = async(page) => {
    await page.goto("https://lampcook.com/food/food_dic_menu4.php");
...
getdata = async() => {
    ...
    var URL = "https://lampcook.com/food/food_dic_menu4.php?pagenum=";
    ...
    for(var i=1; i<=max; i++){
        ...
        if(i==max){
            ...
            name = await page.$eval(`#div_main_content > div.txt_padd_box1.def_color > div > ul > li:nth-child(1) > div`, element => element.textContent.trim());
            data.en = await page.$eval(`#div_main_content > div.txt_padd_box10 > ul > li:nth-child(1)`, element => element.textContent.trim());
            data.jp = await page.$eval(`#div_main_content > div.txt_padd_box10 > ul > li:nth-child(2)`, element => element.textContent.trim().split("\t")[0]);
            data.cn = await page.$eval(`#div_main_content > div.txt_padd_box10 > ul > li:nth-child(3)`, element => element.textContent.trim());
   ...
   // fs.writeFileSync('enfood.json', JSON.stringify(dataset));
    await browser.close();
}
